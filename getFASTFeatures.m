function [output] = getFASTFeatures(I, NrPts, BlockSize)
    ...
    if(size(points,1)<NrPts)
        points = detectFASTFeatures(I,'MinQuality',0,'MinContrast',0.001);
        if(size(points,1)<NrPts)
            points = detectFASTFeatures(histeq(I));
        end
    end
    selPts = points.selectStrongest(NrPts);
