function [output] = getBRISKFeatures(I, NrPts, BlockSize)
    ...
    if(size(points,1)<NrPts)
        points = detectBRISKFeatures(I,'MinQuality',0,'MinContrast',1e-6,'NumOctaves',15);
        if(size(points,1)<NrPts)
            points = detectBRISKFeatures(histeq(I));
        end
    end
